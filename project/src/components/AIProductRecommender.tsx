...
export default function AIProductRecommender() {
  ...
  const recommendations = useMemo(() => {
    const productPool = products.filter((product) => product.category === 'phones' || product.category === 'gaming');
    ...
    const scored = productPool.map((product) => {
      ...
      if (preferences.gaming && (product.category === 'gaming' || specsText.includes('120hz') || specsText.includes('snapdragon'))) {
        score += 14;
      }
      ...
      <article className="frost-panel futuristic-border rounded-3xl p-6 sm:p-8">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-primary sm:text-3xl">Trouve ton smartphone idéal</h2>
            <p className="mt-2 text-sm text-muted">Choisis ton budget et tes priorités, on te recommande les meilleurs modèles.</p>
          </div>
